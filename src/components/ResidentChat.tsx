...
export const ResidentChat: React.FC<ResidentChatProps> = ({ onTicketSelect, isDarkMode, onRefreshTickets }) => {
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      ...
      // REAL Text-to-Speech for AI response
      if ('speechSynthesis' in window) {
        try {
          ...
          // Optional: Detect if response is English and switch voice
          const isEnglish = /^[a-zA-Z\s.,!?]+$/.test(data.replyText || '');
          if (isEnglish) utterance.lang = 'en-IN';
  ...
  const toggleRecording = () => {
    if (!isRecording) {
      ...
      if (SpeechRecognition) {
        ...
        recognition.lang = 'hi-IN'; // Hindi-India, also understands Hinglish
        ...
        recognition.onerror = (event: any) => {
          ...
          setIsRecording(false);
          setInputMessage('Voice input failed. Try typing instead.');
        };

        recognition.onend = () => {
          // Ensure recording state is reset when speech recognition ends
          setIsRecording(false);
        };
      } else {
        ...
        setTimeout(() => {
          setIsRecording(false);
          setInputMessage('Voice feature not supported in this browser. Please type your complaint.');
        }, 1500);
      }
    } else {
      setIsRecording(false);
      ...
      <div className={`p-3 rounded-lg border shadow-2xs ${isDarkMode ? 'bg-[#181817] border-[#2D2D2A]' : 'bg-white border-[#E5E5E1]'}`}>
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            ...
            title={isRecording ? 'Listening voice complaint...' : 'Start Voice Input'}
          >
